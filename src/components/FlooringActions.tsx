...
const FlooringActions: React.FC = () => {
  ...
  const flooringItems = [
    { name: 'SEA SALT', image: '/images/floors/colorSeaSalt.jpg' },
    { name: 'OYSTER', image: '/images/floors/colorOyster.jpg' },
    { name: 'PORCINI', image: '/images/floors/colorPorcini.jpg' },
    { name: 'OAT', image: '/images/floors/colorOat.jpg' },
    { name: 'CARAMEL', image: '/images/floors/colorCaramel.jpg' },
    { name: 'BISCOTTI', image: '/images/floors/colorBiscotti.jpg' },
    { name: 'MAPLE', image: '/images/floors/colorMaple.jpg' },
    { name: 'BONBON', image: '/images/floors/colorBonbon.jpg' },
    { name: 'TRUFFLE', image: '/images/floors/colorTruffle.jpg' },
    { name: 'MOCHA', image: '/images/floors/colorMocha.jpg' },
  ];
     ...
     <section className="relative bg-gray-100 p-8 h-96 flex items-center justify-center">
           ...
           <div className="bg-white p-8 rounded shadow-lg max-w-4xl w-full max-h-full overflow-auto">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
               {flooringItems.map((item, index) => (
                 <div key={index} className="border rounded overflow-hidden cursor-pointer" onClick={() => handleImageClick(item.image)}>
                   <img src={item.image} alt={item.name} className="w-full h-48 object-cover" />
                   <div className=" text-black text-left  py-2" style={{ fontFamily: "'Poppins', sans-serif", marginLeft: "7px" }}>{item.name}</div>
                 </div>
               ))}
             </div>
           </div>
       ...
       {isImageModalOpen && selectedImage && (
         <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30">
           <div className="relative bg-white p-8 rounded shadow-lg max-w-full max-h-full overflow-auto">
             <button 
             ...
             <img src={selectedImage} alt="Selected Flooring" className="w-full h-auto" />
           </div>
         </div>
       )}
     </section>
